...
def dados_viagem(atual,destino,p):
    if p==2 and atual==destino:
        return 0
#passando as cidades
#ajustando as entradas para a url
    atual_ok = atual.replace(' ','+')
    destino_ok = destino.replace(' ','+')
...
# obtendo a distancia em quilometros a ser percorrida
    ...
    resultado = "A distancia percorrida séra de {} km  e a duração estimada é de {} horas e {} minutos".format(distancia,int(horas),int(minutos))
    return resultado
